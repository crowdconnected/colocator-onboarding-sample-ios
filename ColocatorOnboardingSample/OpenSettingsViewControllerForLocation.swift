...
protocol ManualLocationPermissionDelegate {
    func didEnableLocationPermission()
}

class OpenSettingsViewControllerForLocation: UIViewController {
    
    public var delegate: ManualLocationPermissionDelegate?
    ...
    @IBAction func actionOpenSettings(_ sender: Any) {
        ...
        delegate?.didEnableLocationPermission()
        UserDefaults.standard.set(true, forKey: hasAskedForManualLocationPermissionKey)
